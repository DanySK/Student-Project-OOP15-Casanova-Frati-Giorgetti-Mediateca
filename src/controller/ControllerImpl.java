...
public class ControllerImpl implements Controller {
	...
	public void writeOnFile() {
		...
		try {
			...
			this.m.registerMovie("Star Trek", 2009, "Bad Robot", "J.J. Abrams",
					 Language.ENGLISH, ItemGenre.FANTASY, 120, true, 10);
	...
	public void addReview() {
		...
		System.out.println("addReview: score e review " + this.v.getScore()
				+ this.v.getReview());
		/*
		...
		for (Integer i : this.m.getItemArchive().keySet()) {
			try {
				if (this.m.getRequiredItem(i).toString()
						.equals(this.itemBeforeScreenChange)) {
	...
	public void borrowListFromUser(final UserImpl user) {
		...
		System.out.println("borrowList: dimensione archivio prestiti="
	...
	public void setTakenSitsList() {
		...
		for (int index = 0; index < (arrayInt.size()); index++) {
			if ((arrayInt.get(index) == null) || (arrayInt.get(index) == 0)) {
				...
				try {
					array[index] = this.m.getRequiredUser(arrayInt.get(index))
							.getUsername();
					System.out.println("setTakenSitsList: assegnato posto "
							+ index + " a user");
				} catch (ItemException e) {
					this.v.showError(e.getMessage());
				} catch (UserException e1) {
	...
	public void takeSit() {
		...
		try {
			...
			this.m.bookSit(day, this.v.getSelectedSit(),
					this.actualUser.getIdUser());
	...
	public void cancelSit() {
		...
		try {
			...
			this.m.cancelSit(day, this.v.getSelectedSit(),
					this.actualUser.getIdUser());
	...
	public void elementSelectedInManager() {
		...
		for (Integer i : this.m.getItemArchive().keySet()) {
			try {
				if (this.m.getRequiredItem(i).toString()
						.equals(this.v.getUserItemSelectedByManager())) {
					this.setSelectedItemInfo(this.v
